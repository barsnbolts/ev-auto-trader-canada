...
| **6** | Polish & package | Unsigned `.app` in Ian's `/Applications/`, mom-friendly Plain/Geek toggle, accessibility pass, design-elegance sweep, user guide. (Code signing, notarization, auto-update, and `.dmg` distribution were **dropped 2026-04-23** — personal-use tool, not shipped. See `AUTONOMOUS_PLAN.md` Graveyard.) |

Each phase ends with a usable app. You can stop at any phase and still have a tool worth opening.

## 6. Data model

Active data shapes live in [`src/types.ts`](src/types.ts) and are runtime-validated by Zod schemas in [`src/data/schema.ts`](src/data/schema.ts) (as of Cluster A). The seed is a single JSON file (`src/data/seed.json`); migration to SQLite is deferred until dataset size makes it worthwhile (likely past 40 vehicles — see I-item queued in `AUTONOMOUS_PLAN.md`).
