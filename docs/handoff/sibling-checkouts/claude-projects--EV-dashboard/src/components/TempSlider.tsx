...
export function TempSlider({ vehicles }: { vehicles: Vehicle[] }) {
  ...
  return (
    <div className="bg-ink-800 rounded-lg border border-ink-700 p-4 mb-6">
      ...
      {/* HVAC toggle + speed + electricity rate */}
      <div className="flex items-center gap-6 mb-4 flex-wrap">
        ...
        <label className="flex items-center gap-2 text-xs text-ink-300" title="Ontario home-charging rate (blended on/off-peak average ≈ 17¢/kWh)">
          Electricity
          <input
            type="number"
            min={0}
            max={1}
            step={0.01}
            value={electricity_cad_per_kwh}
            onChange={(e) => setElectricityRate(Number(e.target.value))}
            className="bg-ink-700 border border-ink-600 rounded px-2 py-1 w-20 text-ink-100 font-mono"
          />
          <span className="text-ink-500">$/kWh</span>
        </label>
      </div>
