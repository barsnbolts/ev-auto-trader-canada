...
public struct VehicleDetailView: View {
    ...
    private func availabilityRunSummary(_ run: SourceRun) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if run.reviewRecords > 0 {
                Label("\(run.reviewRecords) imported record\(run.reviewRecords == 1 ? "" : "s") still need review before they can publish cleanly.", systemImage: "exclamationmark.circle")
                    .font(.caption)
    ...
    private var availabilitySummary: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("\(listings.count) source-backed listing\(listings.count == 1 ? "" : "s") across \(uniqueDealerCount) dealer\(uniqueDealerCount == 1 ? "" : "s")")
                .font(.caption)
                .foregroundStyle(.secondary)
    ...
    private var emptyAvailabilityMessage: String {
        guard let latestRun else {
            return "This trim is in scope, but there are no source-backed listings yet. Import a research file or check the review queue for low-confidence matches."
        }

        if latestRun.status.localizedCaseInsensitiveContains("fail") {
            return "The latest research refresh failed before listings could publish. Review the warning below, then try again after the source issue is resolved."
        }

        if latestRun.reviewRecords > 0 {
            return "This trim is in scope, but \(latestRun.reviewRecords) imported record\(latestRun.reviewRecords == 1 ? "" : "s") from the latest batch still need review before they can publish as source-backed listings."
        }

        return "This trim is in scope, but there are no published listings yet from the latest successful research batch. Import fresh dealer/OEM evidence if you expect inventory to be available."
    }
