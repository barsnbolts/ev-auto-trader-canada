...
- `Context Safe Area`: `compact_context.py` estimates context load, marks 40-69% as `safe-area`, 70%+ as `danger`, writes `.codex/compact-context.md` and `.codex/safe-area.md`, and tells future runs to avoid broad reads when above 40%.
- `Import Hardening`: CSV parser handles quoted commas/newlines and escaped quotes; strict validation now rejects unknown source types, out-of-scope EREV/gas-only powertrains, suspicious prices, missing/invalid observed dates, and blank confidence notes; merge no longer crashes on preexisting duplicate keys.
- `Import Persistence Proof`: `Tests/CatalogTests/CatalogStoreTests.swift` covers `InventoryStore.importResearchFile` writing accepted listings, research batch counts, rejected review rows, and selected imported listing state against a temporary SQLite database.
- `Visual QA Proof`: `script/build_and_run.sh --verify-ui` now launches the app, requires the process to be alive, and attempts a main-display screenshot under `qa/`; screenshot capture can be blocked by macOS Screen Recording permission, in which case launch proof still succeeds with an explicit warning.
